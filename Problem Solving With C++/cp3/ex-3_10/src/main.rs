...
fn read_exercise_rate() -> f64 {
    let mut line = String::new();
    io::stdin()
        .read_line(&mut line)
        .expect("Failed to read line!");
    match line {
        _ if line.trim() == "Sedentary" => 1.2,
        _ if line.trim() == "Somewhat active" => 1.3,
        _ if line.trim() == "Active" => 1.4,
        _ if line.trim() == "Highly active" => 1.5,
        _ => panic!("Invalid Exercise Status!"),
    }
}
...
fn main() {
    ...
    println!("Enter your exercise(Sedentary, Somewhat active, Active, Highly active): ");
    let exercise_rate = read_exercise_rate();

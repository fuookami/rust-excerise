...
fn main() {
    loop {
        ...
        let gross_pay = match man_hour {
            man_hour if man_hour > NORMAL_MAN_HOUR => ((man_hour - NORMAL_MAN_HOUR) as f64 * OVERTIME_MAN_HOUR_PAY_SCALE + NORMAL_MAN_HOUR as f64) * HOURLY_PAY,
        ...
        let extra_medicial_insurance = match amount {
            _ if amount >= EXTRA_MEDICIAL_INSURANCE_FAMILY_MEMBER_AMOUNT => EXTRA_MEDICIAL_INSURANCE,
            _ => 0.
        };
        let actual_pay = gross_pay - social_insurance - federal_tax - state_tax - WEEKLY_UNION_DUES - extra_medicial_insurance;
        ...
        match amount {
            _ if amount >= EXTRA_MEDICIAL_INSURANCE_FAMILY_MEMBER_AMOUNT => println!("Extra medical insurance: {:.2}", EXTRA_MEDICIAL_INSURANCE),
            _ => {}
        }

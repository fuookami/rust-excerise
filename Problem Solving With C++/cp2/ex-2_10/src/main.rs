...
fn main() {
    ...
    let positive_integers = integers
        .iter()
        .filter(|&x| *x >= 0)
        .cloned()
        .collect::<Vec<i32>>();
    let negative_integers = integers
        .iter()
        .filter(|&x| *x < 0)
        .cloned()
        .collect::<Vec<i32>>();
    ...
    println!(
        "Mean of positivie integers: {}",
        positive_integers_sum / positive_integers.len() as i32
    );
    println!("Sum of negative intergers: {}", negative_integers_sum);
    println!(
        "Mean of negative integers: {}",
        negative_integers_sum / negative_integers.len() as i32
    );
    println!("Sum of all integers: {}", integers.iter().sum::<i32>());
    println!(
        "Mean of all integers: {}",
        integers.iter().sum::<i32>() / integers.len() as i32
    );
}
